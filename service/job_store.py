...
class JobRecord:
    ...
    def from_row(cls, row: JobRow) -> JobRecord:
        ...
        return cls(
            ...
            frontend=getattr(row, "frontend", None),

            backend=getattr(row, "backend", None),

            endpoints=getattr(row, "endpoints", None),

            flow=getattr(row, "flow", None) or "qa",

            flow_inputs=getattr(row, "flow_inputs", None),
        ...
        *,
        ...
        with self._session_factory() as session:
            ...
            row = JobRow(
                ...
                frontend=frontend if frontend is not None else True,
